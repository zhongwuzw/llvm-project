...
namespace {
class ObjcUIViewControllerInitializeOverrideChecker
    : public Checker<check::ASTDecl<ObjCCategoryImplDecl>> {
  mutable std::unique_ptr<BugType> BT;
  mutable IdentifierInfo *IUIViewController;

public:
  void checkASTDecl(const ObjCCategoryImplDecl *D, AnalysisManager &mgr,
                    BugReporter &BR) const;
};
} // namespace

void ObjcUIViewControllerInitializeOverrideChecker::checkASTDecl(
    const ObjCCategoryImplDecl *OCD, AnalysisManager &Mgr,
    BugReporter &BR) const {
  ...
  for (const auto *MD : OCD->methods()) {
    if (MD->isClassMethod() &&
        MD->getSelector().getMethodFamily() == OMF_initialize) {
...
void ento::registerObjcUIViewControllerInitializeOverrideChecker(
    CheckerManager &Mgr) {
  Mgr.registerChecker<ObjcUIViewControllerInitializeOverrideChecker>();
}

bool ento::shouldRegisterObjcUIViewControllerInitializeOverrideChecker(
